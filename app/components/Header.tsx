      ...
      {/* Primary row */}
       <div className="section h-16 md:h-20 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2" aria-label="Pat’s Power Washing — Home">
          {/* Replace with your actual file: /public/logo.svg|.png|.webp */}
          <Image
            src="/logo-light.avif"
            ...
            priority
        />
          <span className="hidden sm:inline font-semibold tracking-tight text-brand-dark">
            Pat’s Power Washing
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          ...
          <Link href="/gallery" className="hover:text-brand-primary">Gallery</Link>
          <Link href="/reviews" className="hover:text-brand-primary">Reviews</Link>
        ...
        >
          {menuOpen ? (
            <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
          ) : (
            <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
          )}
        </button>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-100 bg-white">
          <div className="section py-3 flex flex-col gap-2">
            <Link href="/services" onClick={() => setMenuOpen(false)} className="py-2">Services</Link>
            <details className="[&_summary]:py-2">
              ...
              <ul className="ml-3 mt-1 mb-2 border-l border-slate-200">
                {serviceLinks.map((s) => (
                  <li key={s.href}>
                    <Link href={s.href} onClick={() => setMenuOpen(false)} className="block py-2 pl-3">
                      {s.label}
                    </Link>
                  </li>
                ))}
            ...
            <Link href="/gallery" onClick={() => setMenuOpen(false)} className="py-2">Gallery</Link>
            <Link href="/reviews" onClick={() => setMenuOpen(false)} className="py-2">Reviews</Link>
            <Link href="/contact" onClick={() => setMenuOpen(false)} className="py-2">Contact</Link>
            <div className="h-px bg-slate-200 my-2" />
            <a href="tel:12409684892" className="rounded-xl px-4 py-2 bg-brand-primary text-white text-center shadow">
              Call: 240-968-4892
            </a>
            <Link href="/contact" onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-2 bg-brand-secondary text-white text-center shadow">
              Get a Free Quote
            </Link>
          </div>
        </div>
